...
IP_ADDRESS="0.0.0.0" #$(ip -4 addr show eth0 | grep -oP "(?<=inet).*(?=/)"| sed -e "s/^[[:space:]]*//")
...
wait
echo "${ENDPOINT_URL:-http://apps/api/v1/mqtt}"
vmq-admin plugin disable -n vmq_passwd && \
    vmq-admin plugin disable -n vmq_acl && \
    vmq-admin webhooks register hook=auth_on_register endpoint="${ENDPOINT_URL:-http://apps/api/v1/mqtt}/auth_on_register" && \
    vmq-admin webhooks register hook=auth_on_subscribe endpoint="${ENDPOINT_URL:-http://apps/api/v1/mqtt}/auth_on_subscribe" && \
    vmq-admin webhooks register hook=on_subscribe endpoint="${ENDPOINT_URL:-http://apps/api/v1/mqtt}/on_subscribe" && \
    vmq-admin webhooks register hook=on_client_gone endpoint="${ENDPOINT_URL:-http://apps/api/v1/mqtt}/on_client_gone" && \
    vmq-admin webhooks register hook=on_client_offline endpoint="${ENDPOINT_URL:-http://apps/api/v1/mqtt}/on_client_offline" && \
    vmq-admin webhooks register hook=auth_on_publish endpoint="${ENDPOINT_URL:-http://apps/api/v1/mqtt}/auth_on_publish"
